...
public class UserRepository : IUserRepository
{
    ...
    public List<User> GetAll()
    {
        ...
        con.Open();
        using var cmd = con.CreateCommand();
        cmd.CommandText = "SELECT id, username, is_admin, created_at FROM users ORDER BY id";
    ...
    public User Create(string username, string password, bool isAdmin)
        => CreateWithHash(username, BC.HashPassword(password), isAdmin);

    public User CreateWithHash(string username, string passwordHash, bool isAdmin)
    {
        ...
        return new User { Id = (int)id, Username = username, IsAdmin = isAdmin, CreatedAt = now };
    }
